...
import {TNetworkInfo, TToken} from 'types';

function	Details({tokensData, chainExplorer}: {tokensData: any, chainExplorer: string}): ReactElement {
...
function	Tokens({vault, chainExplorer}: {vault: TToken, chainExplorer: string}): ReactElement {
...
function	Index({tokens}: {tokens: TToken[] | undefined}): ReactElement {
		...
		<section className={'p-4 w-full bg-white dark:bg-black rounded-sm'}>
			...
			<div>
				<div className={'w-full max-w-5xl'}>
					...
					<div className={'w-full'}>
						{(data || [])?.filter((e): boolean => e.hasMissingTokenInfo).map((vault): ReactElement => (
