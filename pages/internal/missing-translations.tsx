...
function Protocol({name, filename, description, missingTranslationsLocales, localeFilter, localization}: TProtocolParams): ReactElement {	
...
// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-ignore
function	Index({protocolsList}): ReactElement {
	...
	function getNetworkInfo(network: string): number {
		switch (network){
		case 'Ethereum': 
			return 1;
		case 'Fantom' : 
			...
			<div>
				<div className={'w-full max-w-5xl'}>
							...
							<div className={'flex flex-col'}>
								<div className={'flex flex-row items-center mb-8 w-full'}>
									...
									<div
										...
										style={{marginTop: -2}}
										onClick={(): Promise<any> => refetchData(getNetworkInfo(currentNetwork))}>
								...
								<div className={'flex flex-row items-center space-x-4'}>
									...
									>
										...
										{Object.entries(LOCALES).map(([locale, localeData], index): ReactElement => (
					...
					<div className={'mt-4 w-full'}>
						...
						{/* // eslint-disable-next-line @typescript-eslint/ban-ts-comment  
						// @ts-ignore */}
						{protocolsWithMissingTranslations.map((protocol): ReactElement => <Protocol key={protocol.name} localeFilter={localeFilter} {...protocol} />)}
					</div>
				</div>
			</div>
		</section>
