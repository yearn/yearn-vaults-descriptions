	...
	if (strategiesData.length === 0) {
		return (
			<section aria-label={'STRATEGIES'}>
				<div className={'w-full text-ygray-200 dark:text-dark-white dark:text-dark-50'}>
					<i
						className={'text-xs'}
					...
					<div key={index} className={'flex flex-col ml-4 md:ml-12 relative'}>
						<div className={'text-ygray-200 dark:text-dark-white mb-4'}>
							<div className={'mb-2 flex flex-row items-center'}>
								...
								<div>
									{strategy.noIPFS ? (
										<span className={'bg-tag-withdraw text-ygray-200 dark:text-dark-white font-bold rounded-md px-2 text-xs py-1 ml-2'}>
											{'Missing IPFS file'}
										</span>
									) : null}
